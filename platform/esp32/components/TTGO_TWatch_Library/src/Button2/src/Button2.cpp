...
void Button2::loop() {
  ...
  // is the button released?
    ...
    if (down_time_ms >= debounce_time_ms) {
      // trigger release        
      if (change_cb != NULL) change_cb ();
      if (released_cb != NULL) released_cb ();
      // trigger tap
      if (tap_cb != NULL) tap_cb ();        
      ...
      switch (click_count) {
        case 1: 
         ...
         case 2: 
          ...
          if (double_cb != NULL) double_cb ();
          break;
         case 3: 
          last_click_type = TRIPLE_CLICK;
          if (triple_cb != NULL) triple_cb ();
          break;
      }
    }
    click_count = 0;
