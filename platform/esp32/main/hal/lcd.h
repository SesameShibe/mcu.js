...
void IRAM_ATTR halLcdUpdate() {
	...
	LCD_SET_DC(1);
	LCD_SET_CS(0);
	halLcdWritePixels((u32*) &lcdFB[2], LCD_WIDTH * LCD_HEIGHT * 2 / 4);
	LCD_SET_CS(1);
}

void halLcdInit() {
	lcdFB = mcujs_alloc_function(0, (LCD_WIDTH * LCD_WIDTH * 2) + (sizeof(uint16_t) * 2));
	...
	memset(lcdFB, 0x0f, (LCD_WIDTH * LCD_HEIGHT * 2) + (sizeof(uint16_t) * 2));
	lcdFB[0] = LCD_WIDTH;
	lcdFB[1] = LCD_HEIGHT;
...
JS_BUFFER halLcdGetFB() {
	JS_BUFFER buf = { .buf = (u8*) lcdFB, .size = (LCD_WIDTH * LCD_HEIGHT * 2) + (sizeof(uint16_t) * 2) };
	return buf;
}